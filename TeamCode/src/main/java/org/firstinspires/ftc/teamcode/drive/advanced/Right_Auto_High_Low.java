...
public class Right_Auto_High_Low extends LinearOpMode {
    ...
    int scored = 0;
    int offset = 0;
    double distance = 60.3;
    double distance_2 = 30;
    double distance_3 = 48.75;
    /**
                    ...
                    if (waitTimer1.seconds() >= waitTime1) {
                        ...
                        if(scored != 6) {
                            Trajectory trajectory4 = drive.trajectoryBuilder(PoseStorage.currentPose)
                                    .lineToLinearHeading(new Pose2d(distance, -17.5, Math.toRadians(0)))
                                    .build();
                    ...
                    if (lift.pos() >= 650-offset) {
                        if (scored < 4) {
                            Trajectory trajectory3 = drive.trajectoryBuilder(PoseStorage.currentPose)
                                    .lineToLinearHeading(new Pose2d(distance_2, -14.75, Math.toRadians(0)))
                                    .build();
                            drive.followTrajectoryAsync(trajectory3);
                        } else {
                            Trajectory trajectory8 = drive.trajectoryBuilder(PoseStorage.currentPose)
                                    .lineToLinearHeading(new Pose2d(distance_3, -22.3, Math.toRadians(0)))
                                    .build();
