...
public class Left_Auto extends LinearOpMode {
    ...
    Pose2d traj3end = new Pose2d(-55, -23, Math.toRadians(65));
    /**
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Trajectory trajectory1 = drive.trajectoryBuilder(startPose)
                .lineToLinearHeading(new Pose2d(-32.5, -41, Math.toRadians(180)))
                .build();
        Trajectory trajectory0 = drive.trajectoryBuilder(trajectory1.end())
                .lineToLinearHeading(new Pose2d(-62.5, -41, Math.toRadians(90)))
                .build();
        //move the robot in a curve around the low junction near the cone stack
        //and lines up our arm to deposit.
        Trajectory trajectory2 = drive.trajectoryBuilder(trajectory0.end())
                .lineToLinearHeading(new Pose2d(-56.5, -24, Math.toRadians(65)))
                .build();

        Trajectory trajectory5 = drive.trajectoryBuilder(traj3end)
                .lineToLinearHeading(new Pose2d(-58.75, -36, Math.toRadians(90)))
                .build();

        Trajectory trajectory6 = drive.trajectoryBuilder(traj3end)
                .splineTo(new Vector2d(-33, -16), Math.toRadians(0))
                .build();

        Trajectory trajectory7 = drive.trajectoryBuilder(traj3end)
                .splineTo(new Vector2d(-36, -16), Math.toRadians(0))
                .splineTo(new Vector2d(-8, -18), Math.toRadians(0))
                .build();
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            switch (currentState) {
                ...
                case TURRET:
                    ...
                    if (Math.abs(turret.pos())>Math.abs(turret_target)-35 && Math.abs(turret.pos())<Math.abs(turret_target)+10) {
                    ...
                    if (waitTimer1.seconds() >= waitTime1) {
                        ...
                        if(scored != 5) {
                            Trajectory trajectory4 = drive.trajectoryBuilder(PoseStorage.currentPose)
                                    .lineToLinearHeading(new Pose2d(distance, -15, Math.toRadians(90)))
                                    .build();
                ...
                case ROTATE:
                    ...
                    // If not, move onto the next state, IDLE
                    // We are done with the program
                    if (Math.abs(turret.pos())>Math.abs(turret_target)-50 && Math.abs(turret.pos())<Math.abs(turret_target)+10) {
                        score = false;
                ...
                case WAIT_3:
                    ...
                    if (lift.pos() >= 650-offset) {
                        ...
                        Trajectory trajectory3 = drive.trajectoryBuilder(PoseStorage.currentPose)
                                .lineToLinearHeading(new Pose2d(-55, -22.5, Math.toRadians(65)))
                                .build();
